...
var dbConfig = require('../../config/index.js');
...
function excuteSql(arg0, arg1, arg2){
  ...
  if(!checkArgSingle(arg0, arg1, arg2)){
    return callback('Arg invalid, db.excuteSql', errCodes.ARG_CHECK_FAILED);
  }
  if(para){
...
function recordExists(arg0, arg1, arg2){
  ...
  if(para){
    ...
    logger.debug(sql);
    logger.debug(para);
    excuteSql(sql, para, function(err1, rows1){
      return getExistsResult(err1, rows1, callback);
    });
  }else{
    excuteSql(sql, function(err2, rows2){
...
function checkUniqueMultiFields(conf, callback){
  ...
  logger.debug('pppppppppppppppppppppppppppppppppppppppppppppp');
  logger.debug(conf);
  logger.debug(sql);
