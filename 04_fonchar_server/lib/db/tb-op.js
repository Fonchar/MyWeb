...
//id int auto_increment primary key
  ...
  function getIdByField(fieldName, fieldValue, callback){
    ...
    db.executeSql(sql, [fieldValue], function(err, rows){
      if(err){
        return callback(err);
      }
      if(!rows.length){
	...
	function add(obj, callback) {
		...
		db.executeSql(sqlAdd(), paraValues, function(err, rows) {
			if(err){
				return callback(err);
			}
	...
	function addIfNotExists(obj, repeatCheckFieldNamesInDb, repeatCheckFieldNamesInObj, callback){
		...
		db.executeSql(sqlAddIfNotExists(repeatCheckFieldNamesInDb), paraValues, function(err, rows) {
			if(err){
				return callback(err);
			}
	...
	function update(obj, callback) {
		...
		db.executeSql(sqlUpdate(), paraValues, function(err, rows) {
			if(err){
				return callback(err);
			}
			callback(null, {});
	...
	function remove(obj, callback) {
		db.executeSql(sqlRemove(), [obj.id], function(err, rows) {
			if(err){
				return callback(err);
			}
			callback(null, {});
	...
	function show(obj, callback) { // 通过id查单条记录
		db.executeSql(sqlShow(), [obj.id], function(err, rows) {
			if(err){
				return callback(err);
			}
			if(rows.length == 0) {
	...
	function queryAll(obj, callback) {
		db.executeSql(sqlQueryAll(), function(err, rows) {
			if(err){
				return callback(err);
			}
